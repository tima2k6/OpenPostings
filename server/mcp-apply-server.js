...
async function findCandidates(options = {}) {
  ...
  return {
    ...
    items: result.items,
    // Present only when this call needed the wide-scan walk (search/structured filters/
    // review_queue/match ranking). true means the walk gave up after scanning a large bound
    // of candidates without filling this page -- a short or empty result then means "narrow
    // the filters" (fewer OR'd industries, a tighter search term), not "nothing matches".
...
async function main() {
  ...
  mcpServer.registerTool(
    ...
    {
      description:
        "Read applicantee information, MCP settings and the run playbook. Also lists the stored documents and the state of any file paths configured for them, so an unreadable document is known up front rather than surfacing as an error part-way through drafting an application. match_scoring reports the background match-index job's progress per resume key -- check it before trusting find_posting_candidates' match_scores/match_percent, since a freshly uploaded resume is scored incrementally against the whole corpus and can take a while to catch up (caught_up: false, corpus_coverage_percent well under 100) rather than being wrong or broken. Contains no credentials: the agent prepares applications and hands off at the authentication boundary."
    },
    async () => {
      ...
      return asToolResult({
        ...
        // last_run_at/scored_count/corpus_coverage_percent per resume key -- see the tool description.
